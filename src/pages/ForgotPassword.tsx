...
function StepRequest({ onSent }: { onSent: (email: string) => void }) {
  ...
  return (
    <>
      <h1 className="text-3xl font-bold text-midnight dark:text-ink mb-1">Esqueceu sua senha?</h1>
...
function StepConfirm({ email, onDone, onBack }: { email: string; onDone: () => void; onBack: () => void }) {
  ...
  return (
    <>
      <h1 className="text-3xl font-bold text-midnight dark:text-ink mb-1">Redefinir senha</h1>
      <p className="text-sm text-ink-muted mb-6">
        Mandamos um código de 6 dígitos para <strong className="text-midnight dark:text-ink">{email}</strong>
      </p>
        ...
        <div className="flex justify-center gap-2" onPaste={handlePaste}>
          {code.map((digit, i) => (
            <input
              ...
              disabled={submitting}
              className="w-11 h-13 text-center text-xl font-bold border-2 border-border rounded-xl focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 outline-none transition-all disabled:opacity-50"
            />
          ))}
        </div>
      ...
      <div className="flex items-center justify-between mt-4 text-sm">
        <button onClick={onBack} className="flex items-center gap-1 text-ink-muted hover:text-midnight dark:hover:text-ink">
...
function StepDone({ email }: { email: string }) {
  return (
    <div className="text-center">
      ...
      <h1 className="text-3xl font-bold text-midnight dark:text-ink mb-1">Senha redefinida</h1>
      <p className="text-sm text-ink-muted mb-8">Sua senha foi alterada com sucesso. Entre com sua nova senha.</p>
